...
* When you have completed the assignment, push it to your personal repo providing access only to a list of users. This list will be provided to you separately.
...
* `coffeebeans_dataeng_exercise/ingest.py` -  the entry point for running the ingestion process.
* `coffeebeans_dataeng_exercise/outliers.py` - the entry point for running the outlier detection query.
* `coffeebeans_dataeng_exercise/db.py` - is empty, but the associated test demonstrates interaction with an DuckDB database.
...
`uncommitted/votes.jsonl` contains the input data.
