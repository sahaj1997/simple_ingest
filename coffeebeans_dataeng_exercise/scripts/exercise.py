...
@app.command()
def tidy():
    run_cmd("isort coffeebeans_dataeng_exercise")
    run_cmd("autopep8 --in-place --recursive coffeebeans_dataeng_exercise")


@app.command()
def lint():
    run_cmd("mypy coffeebeans_dataeng_exercise")
    run_cmd("flake8 coffeebeans_dataeng_exercise")


@app.command()
def test():
    run_cmd(
        "pytest --cov=coffeebeans_dataeng_exercise coffeebeans_dataeng_exercise tests "
...
def ingest_data():
    ...
    run_cmd(f"python -m coffeebeans_dataeng_exercise.ingest {path_to_data}")
...
@app.command()
def detect_outliers():
    run_cmd("python -m coffeebeans_dataeng_exercise.outliers")
